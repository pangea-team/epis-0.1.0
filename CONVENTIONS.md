...
`[PROJECT_NAME]`의 코딩 컨벤션 문서입니다. Biome로 자동화되지 않는, 팀 합의가 필요한 영역을 정리합니다.
...
- 라우트 경로는 하드코딩하지 않고 상수로 관리할 것. 동적 경로는 함수 형태로 호출.
...
**우선순위:**
1. 시맨틱 클래스 우선: `.text-display`, `.text-heading-{sm,md,lg}`, `.text-body-{sm,md,lg}`, `.text-label-{sm,md}`, `.text-caption`, `.text-quote`
...
2. 커스텀 변수: `text-(--color-text-tertiary)` 등 (`styles/colors.css` 참조)
